...
class ProjectRepository extends BaseRepository
{
    ...
    public function getSearchedProjectsForListing(ProjectsListingRequestDto $projectsListingRequestDto)
    {
        $qb = $this->createQueryBuilder('p')
            ->select(
                'p.id as project_id',
        ...
        if ($projectsListingRequestDto->getSearchedText() !== null) {
            $qb->andWhere('(LOWER(p.title) LIKE :searchedText OR LOWER(p.shortDescription) LIKE LOWER(:searchedText))')
                ->setParameter('searchedText', '%' . $projectsListingRequestDto->getSearchedText() . '%');
        }

        if ($projectsListingRequestDto->getCategoryId() !== null) {
            $qb->andWhere('category.id = :categoryId')
                ->setParameter('categoryId', $projectsListingRequestDto->getCategoryId());
        }
